...
import { ArrowLeft, Clock, ChefHat, Heart, Share2, Users } from "lucide-react";
          ...
          <div className="flex gap-3 max-w-lg mx-auto">
            ...
            {currentStep < recipe.steps.length - 1 ? (
              ...
              <Button
                ...
                className="flex-1 gradient-cool text-secondary-foreground border-0"
              >
                Done! 🎉
              </Button>
            )}
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="relative h-72">
        ...
        <button
          onClick={() => navigate(-1)}
          className="absolute top-4 left-4 w-9 h-9 rounded-full bg-background/30 backdrop-blur flex items-center justify-center"
        >
            ...
            <div className="space-y-2">
              {recipe.steps.map((step, i) => (
                <div key={i} className="flex gap-3">
                  <span className="w-6 h-6 rounded-full bg-primary/20 text-primary text-xs flex items-center justify-center flex-shrink-0 font-bold">
                    {i + 1}
                  </span>
      ...
      <div className="fixed bottom-16 left-0 right-0 bg-card/95 backdrop-blur-lg border-t border-border p-3">
        <div className="flex gap-2 max-w-lg mx-auto">
          ...
          <Button
            variant="outline"
            onClick={() => navigate(`/cook-together?recipe=${recipe.id}`)}
            className="border-border text-foreground"
          >
            <Users className="w-4 h-4 mr-1" /> Cook Together
          </Button>
